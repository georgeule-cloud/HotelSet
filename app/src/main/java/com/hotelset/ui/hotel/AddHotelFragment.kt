...
import com.hotelset.utiles.AudioUtiles
import com.hotelset.utiles.ImagenUtiles
...
class AddHotelFragment : Fragment() {
    ...
    private lateinit var audios: AudioUtiles

    private lateinit var images: ImagenUtiles
    ...
    private fun subeAudioNube() {
        ...
        if (audioFile.exists() && audioFile.isFile && audioFile.canRead()) {
            ...
            val rutaNube = "hotelset/${Firebase.auth.currentUser?.email}/audios/${audioFile.name}"
    ...
    //         val
    // rutaNube="hotelset/${Firebase.auth.currentUser?.email}/imagenes/${imagenFile.name}"
    //         val referencia: StorageReference = Firebase.storage.reference.child(rutaNube)
    ...
    private fun addHotel(rutaAudio: String, rutaImagen: String) {
        ...
        if (name.isNotEmpty()) {
            ...
            // val tipo = binding.dropdowntipo.text.toString().toDouble()
            val hotel =
                    Hotel(
                            "",
                            name,
            ...
            Toast.makeText(requireContext(), getString(R.string.msg_add_hotel), Toast.LENGTH_LONG)
                    .show()
